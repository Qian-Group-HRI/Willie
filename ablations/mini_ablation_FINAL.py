# ============================================================================
#  WILLIE MINI - FINAL ABLATION (single file, single run)
...
#  Output: ablation_final/  (self-contained, does not touch prior runs)
#  Recipe identical to paper MINI (50 ep, freeze 5, alpha zero-init).
# ============================================================================
#!/usr/bin/env python
"""
================================================================================
  mini_ablation.py  -  WILLIE-MINI Component Ablation (standalone)
  Configs: full / no_WACSA / no_MoE / no_WTCS   x   5 folds
  Saves to: ablation/mini/ckpts , ablation/results/mini_results.json
  Atomic per-config + per-fold resume. Run via sbatch (no notebook needed).
================================================================================
"""
...
CKPT_DIR         = ROOT / "artifacts" / "09_fuseg_csd"   # for the shared 5-fold splits file
SPLITS_FILE      = CKPT_DIR / "5fold_splits_v2.pt"

# NEW organized ablation outputs
ABL_DIR     = ROOT / "ablation_final" / "mini" / "ckpts"
ABL_RESULTS = ROOT / "ablation_final" / "results" / "mini_final_results.json"
...
ABL_CFG = {"epochs":50, "freeze_epochs":5, "lr_head":1e-4, "lr_backbone":1e-5,
           "weight_decay":1e-4, "patience":12, "grad_clip":1.0,
           "accumulation_steps":2, "seg_size":512, "batch_size":4, "num_workers":4}
...
log(f"{'='*80}\n  mini_ablation.py  |  {time.strftime('%Y-%m-%d %H:%M:%S')}\n{'='*80}")
...
def get_model_config(variant="MINI"):
    return {"MINI": ModelConfig(name="MINI", backbone_name="dinov2_vits14", backbone_dim=384,
                                fpn_dim=256, wa_csa_layers=1, num_experts=2, top_k_experts=2)}[variant]
...
class FeaturePyramidNeck(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        D, Fd = cfg.backbone_dim, cfg.fpn_dim
        self.laterals = nn.ModuleList([nn.Sequential(nn.Conv2d(D, Fd, 1, bias=False), nn.GroupNorm(32, Fd), nn.GELU()) for _ in range(cfg.fpn_levels)])
    ...
    def forward(self, feats):
        lats = [self.laterals[i](feats[i]) for i in range(len(feats))]
        pyr = [None]*len(feats); pyr[-1] = lats[-1]
        for i in range(len(feats)-2, -1, -1):
            up = F.interpolate(pyr[i+1], size=lats[i].shape[-2:], mode="bilinear", align_corners=False)
            pyr[i] = lats[i] + up
        return [self.smooth[i](pyr[i]) for i in range(len(feats))]
...
class WILLIEModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.encoder=DINOv2MultiScale(cfg); self.fpn=FeaturePyramidNeck(cfg); self.wa_csa=WA_CSA_Stack(cfg)
    ...
    def unfreeze_backbone(self):
        self.encoder.set_frozen(False); log("  backbone unfrozen")
...
class CheckpointManager:
    ...
    def save(self, model, optimizer=None, scheduler=None, epoch=0, fold=0, metrics=None, tag="latest"):
        ck={"model_state_dict":model.state_dict(),"epoch":epoch,"fold":fold,"metrics":metrics or {},
            "config":model.cfg.__dict__ if hasattr(model,'cfg') else {}}
...
log(f"  loaded splits: {SPLITS_FILE.name}")
...
def parse_yolo(bbox_str):
    if pd.isna(bbox_str) or str(bbox_str).strip() in ("","[]","nan"): return np.zeros((0,4),dtype=np.float32)
    try: raw=ast.literal_eval(str(bbox_str))
...
def combined_metric(ca, sd, ap):
    return {"cls_acc":ca,"seg_dice":sd,"det_ap50":ap,"combined_weighted":0.4*ca+0.4*sd+0.2*ap}
...
def patch_model(model, config):
    if config=="no_WACSA":
        ...
        for film in model.seg_decoder.films:
            film.forward=(lambda self, feat, embed: feat).__get__(film, type(film))
    elif config=="no_MoE":
        dec=model.cls_decoder
        def single(pyr):
            B=pyr[0].shape[0]
            pooled=torch.stack([dec.pools[i](pyr[i]) for i in range(dec.cfg.fpn_levels)],1)
            sw=dec.scale_attn(torch.ones(B,dec.cfg.fpn_levels,device=pooled.device))
            fused=dec.pre((pooled*sw.unsqueeze(-1)).sum(1))
            dec.router.aux_loss=torch.tensor(0.0,device=fused.device)
            return dec.experts[0](fused), dec.embed_head(fused)
        dec.forward=single
...
def train_fold(config, fold):
    cfg=get_model_config("MINI"); m=WILLIEModel(cfg).to(DEVICE); patch_model(m,config)
    crit=MultiTaskLoss(NUM_CLASSES).to(DEVICE)
    ck=CheckpointManager(ABL_DIR/config, f"mini_{config}")
    tl,vl=get_fold_loaders(fold, ABL_CFG["batch_size"])
    bb=list(m.encoder.parameters()); dec=[p for n,p in m.named_parameters() if "encoder" not in n]
    opt=torch.optim.AdamW([{"params":dec,"lr":ABL_CFG["lr_head"]},
                           {"params":list(crit.parameters()),"lr":ABL_CFG["lr_head"]},
                           {"params":bb,"lr":ABL_CFG["lr_backbone"]}], weight_decay=ABL_CFG["weight_decay"])
    ...
    for epoch in range(start, ABL_CFG["epochs"]):
        if epoch==ABL_CFG["freeze_epochs"]: m.unfreeze_backbone()
...
#  MAIN LOOP
# ==============================================================================
CONFIGS=["full","no_WACSA","no_MoE","no_WTCS","backbone_only"]
...
log(f"\n{'='*70}\n  MINI ABLATION SUMMARY (5-fold mean+/-std)\n{'='*70}")
def st(cfg,key):
...
def final_summary():
    ...
    full_m, full_s, full_n = stat("full")
    if full_m is None:
        log("  (no full results yet)"); return
    log("\n"+"="*70)
    log("  MINI FINAL ABLATION  -  Individual vs Collective")
    log("="*70)
    log(f"  {'Config':14s} {'Combined':>12} {'Delta':>9}  Verdict")
    log(f"  {'full':14s} {full_m:7.1f}+/-{full_s:.1f}   {'--':>7}  baseline")
    order=["no_WACSA","no_MoE","no_WTCS","backbone_only"]
    for c in order:
        m,s,n = stat(c)
        if m is None: continue
        d = m-full_m
        if c=="backbone_only":
            verdict = "COLLECTIVE WIN" if abs(d)>full_s else "no collective effect (!)"
        else:
            verdict = "individually redundant" if abs(d)<=full_s else "individual effect"
        log(f"  {c:14s} {m:7.1f}+/-{s:.1f}   {d:+7.2f}  {verdict}")
    log("="*70)
    bo_m,_,_ = stat("backbone_only")
    if bo_m is not None:
        log(f"  COLLECTIVE component contribution: {full_m-bo_m:+.1f} points "
            f"(full {full_m:.1f} vs backbone-only {bo_m:.1f})")
        log(f"  Story: single removals within noise (redundant); all-off drops "
            f"{full_m-bo_m:.1f} pts (collectively essential).")
    log("="*70)
